...
/**
  ...
  * @retval None
  */
extern void SPI_START(void)//低电平启动
{
...
/**
  ...
  * @retval None
  */
extern void SPI_STOP(void)//高电平结束
{
	HAL_GPIO_WritePin(GPIOB,SPI_SCK_PIN,GPIO_PIN_SET);
}

/**
  * @brief  SPI交换一个字节(SPI模式0)
...
uint8_t MySPI_SwapByte(uint8_t ByteSend)
{
    ...
    for (i = 0; i < 8; i++)
    {
        ...
        if (MySPI_R_MISO() == 1) { ByteReceive |= (0x80 >> i); }//移入数据
