...
extern auto_shoot_t auto_shoot; //自瞄数据
...
int press_right_flag = 0, press_right_flag_last = 0; //鼠标左键标志位
int servo_flag = 0;
static uint8_t move_flag = 0;
int speed_cut = 0;//检测摩擦轮超速降速
float shoot_debug_buttle[5] = {0};
...
void fire_task(void const *pvParameters)
{
  ...
  while (1)
  {
    ...
    speed[2] = -shoot_control.trigger_motor.speed * 10;
    speed[3] = shoot_control.trigger_motor.speed_set * 10;
...
static void shoot_set_mode(void)
{
  ...
  else if (switch_is_mid(rc_ctrl.rc.s[SHOOT_RC_MODE_CHANNEL]))
  {
    ...
    if (press_left_flag - press_left_flag_last == 1)
    {
      shoot_control.shoot_mode = SHOOT_BULLET;
		...
		//如果处于中档并按下鼠标右键，进入发射模式
		 if(gimbal_behaviour == GIMBAL_AUTO	&& auto_shoot.fire_flag == 1)
		 {
			 shoot_control.shoot_mode = SHOOT_BULLET;
		 }
  }
  ...
  get_shoot_heat0_limit_and_heat0(&shoot_control.heat_limit, &shoot_control.heat);
	...
	if(shoot_control.heat_limit <= 150)
	{
	...
	{
		if((shoot_control.heat + SHOOT_HEAT_REMAIN_VALUE_H >= shoot_control.heat_limit))
		{
			if (shoot_control.shoot_mode == SHOOT_BULLET || shoot_control.shoot_mode == SHOOT_CONTINUE_BULLET)
			{
				shoot_control.shoot_mode = SHOOT_READY;
  ...
  //获取速度，超速降低摩擦轮转速
  get_shoot_speed(&shoot_control.bullet_speed);
	shoot_debug_buttle[0] = shoot_control.bullet_speed;
  if(!toe_is_error(REFEREE_TOE) && BULLET_SPEED_LIMIT <= shoot_control.bullet_speed && shoot_control.bullet_last_speed != shoot_control.bullet_speed)
  {
     speed_cut += 30;
  }
...
static void shoot_control_loop(void)
{
  if (shoot_control.shoot_mode == SHOOT_STOP)
  {
    ...
    shoot_control.trigger_motor.given_current = 0.0f;
	...
	else if(shoot_control.shoot_mode == SHOOT_READY && gimbal_behaviour != GIMBAL_AUTO)
	{
		//设置拨弹轮的速度
    shoot_control.trigger_motor.speed_set = 0.0f;
  ...
  else if ((shoot_control.shoot_mode == SHOOT_READY && gimbal_behaviour == GIMBAL_AUTO) || shoot_control.shoot_mode == SHOOT_BULLET || shoot_control.shoot_mode == SHOOT_CONTINUE_BULLET)
  {
    //设置拨弹轮的速度
    shoot_control.trigger_motor.speed_set = 0.0f;
    //摩擦轮缓启动
    if (shoot_control.right_fricition_motor.speed > (FRICTION_SPEED_SET * 0.8f))
    {
    ...
    //摩擦轮转速设置
    shoot_control.left_fricition_motor.speed_set = -(FRICTION_SPEED_SET - speed_cut);
    shoot_control.right_fricition_motor.speed_set = FRICTION_SPEED_SET - speed_cut;
    ...
    if (shoot_control.shoot_mode == SHOOT_BULLET)
    {
      ...
      shoot_control.trigger_motor.speed_set = TRIGGER_SPEED;
			...
			shoot_control.trigger_motor.speed_set = CONTINUE_TRIGGER_SPEED_H;
...
static void shoot_bullet_control(void)
{
  ...
  {
		//热量限制以上自瞄在双发
		if(gimbal_behaviour == GIMBAL_AUTO && shoot_control.heat_limit >= 150)
    shoot_control.trigger_motor.set_angle = shoot_control.trigger_motor.angle - 2500.0f;
		else
...
static void servo_init()
{
  ...
  __HAL_TIM_SetCompare(&htim1, TIM_CHANNEL_4, 1650); //600
}
