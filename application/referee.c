...
frame_header_struct_t referee_receive_header;//裁判系统接收数据帧头
...
ext_robot_command_ remote_control_t;
...
void referee_data_solve(uint8_t *frame)
{
    ...
    switch (cmd_id)
    {
    case GAME_STATE_CMD_ID://比赛状态数据，1Hz发送
    {
        memcpy(&game_state, frame + index, sizeof(ext_game_state_t));
    }
    break;
    case GAME_RESULT_CMD_ID://比赛结果数据，比赛结束后发送
    {
        memcpy(&game_result, frame + index, sizeof(game_result));
    }
    break;
    case GAME_ROBOT_HP_CMD_ID://血量数据，1Hz发送
    {
    ...
    break;
    case SUPPLY_PROJECTILE_ACTION_CMD_ID://场地补给站动态标识数据
    {
...
void get_chassis_power_and_buffer_and_power_limit(float *power, uint16_t *buffer, uint16_t *power_limit, uint16_t *chassis_output)
{
    ...
    *power_limit = robot_state.chassis_power_limit;
	  *chassis_output = robot_state.mains_power_chassis_output;
}
